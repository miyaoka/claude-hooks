...
ルールは配列として定義される。配列の各要素は以下のフィールドを持つ：

- `command`（オプション）: コマンド名。省略時は全コマンドに適用
- `args`（オプション）: 正規表現パターン
- `reason`: 理由の説明文
...
# evaluator.sh を直接実行
echo '{"command": "rm", "args": "-rf /tmp/test", "description": "test"}' | ./hooks/pretooluse/bash/evaluator.sh
# 出力: {"decision": "block", "reason": "強制削除は危険"}

echo '{"command": "rm", "args": "/tmp/test", "description": "test"}' | ./hooks/pretooluse/bash/evaluator.sh
# 出力: {}
```
